...
const Page = () => {
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    const uniqueSpells = updatedWizard.spells.filter(
      (spell, index, self) =>
    ...
    const warband: Member[] =
      hasApprentice && updatedApprentice
        ? [updatedWizard, updatedApprentice]
        : [updatedWizard];
  ...
  useEffect(() => {
    ...
    const neutralSpellsFound = (
      Array.isArray(foundSchool.neutral)
        ? Gamecodex.spells.filter((spell) =>
            foundSchool.neutral.includes(spell.school),
          )
        : []
    ).map((s) => ({ ...s, targetNumber: s.targetNumber + 4 }));

    const sortedNeutral = neutralSpellsFound.sort(
      (a, b) => a.school - b.school,
    );
    ...
    <main className="min-h-screen w-full pb-20 px-4">
      ...
      >
        ...
        <div className="flex flex-col gap-4">
          <label className="block font-semibold text-[var(--accent-color)] text-lg border-b border-white/10 pb-2">
            School Spells
          </label>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[0, 1, 2].map((index) => (
              <div key={index} className="flex items-center gap-2">
                ...
                <button
                  onClick={() => toggleSpellInfo(index)}
                  type="button"
                  className="w-8 h-8 flex items-center justify-center rounded-full border border-white/20 text-white hover:bg-white/10 hover:border-[var(--accent-color)] hover:text-[var(--accent-color)] transition-all"
                >
                  ?
                </button>
              </div>
          ...
          {/* ALIGNED SPELLS */}
          <label className="block font-semibold text-[var(--accent-color)] text-lg border-b border-white/10 pb-2 mt-4">
            Aligned Spells
          </label>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              ...
              <div key={group.index} className="flex items-center gap-2">
                ...
                <button
                  onClick={() => toggleSpellInfo(group.index)}
                  type="button"
                  className="w-8 h-8 flex items-center justify-center rounded-full border border-white/20 text-white hover:bg-white/10 hover:border-[var(--accent-color)] hover:text-[var(--accent-color)] transition-all"
                >
                  ?
            ...
            {[0, 1].map((i) => (
              <div key={i} className="flex items-center gap-2">
                <select
                  ...
                  required
                  onChange={(e) => {
                    (handleSpellSelect(e, 6 + i), handleNeutralChoice(e, i));
                  }}
                >
                  <option value="" className="text-black">
                    - Select Neutral Spell -
                  </option>
                  {neutralSpells.map((spell) => (
                    <option
                      ...
                      className="text-black"
                    >
                      {spell.name} - {spell.schoolName} (TN:{" "}
                      {spell.targetNumber})
                    </option>
                  ))}
                </select>
                <button
                  onClick={() => toggleSpellInfo(6 + i)}
                  type="button"
                  className="w-8 h-8 flex items-center justify-center rounded-full border border-white/20 text-white hover:bg-white/10 hover:border-[var(--accent-color)] hover:text-[var(--accent-color)] transition-all"
                >
                  ?
          ...
          <div className="flex items-center justify-center gap-3 mt-6 p-4 bg-white/5 rounded-lg border border-white/10">
            <input
              type="checkbox"
              checked={hasApprentice ?? false}
              onChange={toggleHasApprentice}
              className="w-5 h-5 accent-[var(--accent-color)]"
            ></input>
            <p className="text-white font-medium">
              Recruit an Apprentice? (200gc)
            </p>
          </div>
      ...
      {isSpellModalOpen && spellToShow ? (
        <SpellCard
          spell={spellToShow}
          onClick={() => setSpellModalIsOpen(false)}
        />
      ) : null}
    </main>
  );
