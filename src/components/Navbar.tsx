        ...
        <nav className="relative flex justify-between items-center px-6 py-4 glass-panel w-full z-50 mb-6">
            ...
            <div className="flex items-center gap-3">
                {!isPlayMode ?
                    <button
                        type="button"
                        onClick={() => { router.push('/PlayPage'), setIsPlayMode(prev => !prev), setIsOpen(false) }}
                        className="px-4 py-2 rounded-lg bg-[var(--accent-color)] text-slate-900 font-bold hover:bg-[var(--accent-hover)] transition-all duration-200 shadow-lg hover:shadow-[0_0_15px_rgba(56,189,248,0.5)]"
                    >
                        Play Mode
                    </button> :
                    <button
                        type="button"
                        onClick={() => { router.push('/'), setIsPlayMode(prev => !prev), setIsOpen(false) }}
                        className="px-4 py-2 rounded-lg bg-red-500 text-white font-bold hover:bg-red-600 transition-all duration-200 shadow-lg"
                    >
                        Edit Mode
                    </button>}
                ...
                <div className="hidden md:flex gap-2">
                    <button
                        type="button"
                        onClick={() => { goToWizardMenu(), setIsOpen(false) }}
                    ...
                    <button
                        type="button"
                        onClick={() => { router.push('/YourSpells'), setIsOpen(false) }}
                        className="px-4 py-2 rounded-lg border border-[var(--glass-border)] text-white hover:bg-white/10 transition-all duration-200"
                    >
                        Spells
                    </button>
                    ...
                    <div className="absolute top-full right-0 mt-2 w-56 rounded-xl glass-panel overflow-hidden z-50 bg-[#1e293b] border border-white/10 shadow-xl">
                        <div className="py-2 text-gray-200" role="menu">
                            <div className="md:hidden border-b border-gray-700 mb-2 pb-2">
                                <button onClick={() => { goToWizardMenu(); setIsOpen(false) }} className="block w-full px-4 py-2 text-sm text-left hover:bg-white/10 hover:text-[var(--accent-color)]">Wizard Menu</button>
                                <button onClick={() => { router.push('/YourSpells'); setIsOpen(false) }} className="block w-full px-4 py-2 text-sm text-left hover:bg-white/10 hover:text-[var(--accent-color)]">Spells</button>
                            </div>
                            
                            <a href="/EditSpells" className="block px-4 py-2 text-sm text-left hover:bg-white/10 hover:text-[var(--accent-color)]" role="menuitem">Edit Spells</a>
                            <a href="/WarbandBuilder" className="block px-4 py-2 text-sm text-left hover:bg-white/10 hover:text-[var(--accent-color)]" role="menuitem">Add a Member</a>
                            <a href="/AllSpells" className="block px-4 py-2 text-sm text-left hover:bg-white/10 hover:text-[var(--accent-color)]" role="menuitem">All Spells</a>
                            <button
